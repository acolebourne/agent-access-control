...
case class EmpRef(taxOfficeNumber: String, taxOfficeReference: String) extends TaxIdentifier {
  ...
  require(EmpRef.isValid(taxOfficeNumber, taxOfficeReference), s"Invalid EmpRef [$taxOfficeNumber/$taxOfficeReference] - valid format is '^[0-9]{3}/[A-Z0-9]+$$'")
...
object EmpRef {
  ...
  private val taxOfficeNumberFormat = """^[0-9]{3}$""".r
  private val taxOfficeReferenceFormat = "^[A-Z0-9]+$".r

  def isValid(taxOfficeNumber: String, taxOfficeReference: String): Boolean = {
    (taxOfficeNumber, taxOfficeReference) match {
      case (taxOfficeNumberFormat(), taxOfficeReferenceFormat()) => true
      case _ => false
    }
  }
}
