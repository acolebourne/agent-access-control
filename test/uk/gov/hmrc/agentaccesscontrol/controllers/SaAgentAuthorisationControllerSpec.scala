...
class SaAgentAuthorisationControllerSpec extends UnitSpec with BeforeAndAfterEach with MockitoSugar {

  val fakeRequest = FakeRequest("GET", "/")

  val saAgentAuthorisationService = mock[SaAgentAuthorisationService]
  val controller = new SaAgentAuthorisationController(saAgentAuthorisationService)


  override protected def beforeEach(): Unit = {
    super.beforeEach()
    reset(saAgentAuthorisationService)
  }
  ...
  def whenAuthorisationServiceIsCalled =
    when(saAgentAuthorisationService.isAuthorised()(any[ExecutionContext], any[HeaderCarrier]))
}
